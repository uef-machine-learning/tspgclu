...
DataSet *loadSetData(std::string infname) {
  ...
  // line = "foo foo  bar baz blep    bar blep";
...
double dice_set_distance(DataSet *sd, int a, int b) {
  double d =
      (double)(1.0 - dice_ngram(sd->bigrams[a], sd->bigrams[b], sd->setSize[a], sd->setSize[b]));
  // printf("type=%d d(%d,%d)=%f\n",sd->type,a,b,d);
  return d;
}

double dice_distance_precalc(DataSet *sd, int a, int b) {

  int strlen1 = sd->strings->at(a).length();
  int strlen2 = sd->strings->at(b).length();
  // printf("strlen1=%d strlen2=%d\n",strlen1,strlen2);
  double bigram =
      (double)(1.0 - dice_ngram(sd->bigrams[a], sd->bigrams[b], strlen1 - 1, strlen2 - 1));
  double trigram =
      (double)(1.0 - dice_ngram(sd->trigrams[a], sd->trigrams[b], strlen1 - 2, strlen2 - 2));

  double ret = trigram * 0.9 + bigram * 0.1;
  return ret;
}
...
double dice_distance(const std::string &s1, const std::string &s2) {
  //    double ret = (double) (1.0-dice_match(s1.c_str(), s2.c_str()));
  //  double ret = (double)(1.0 - dice(s1.c_str(), s2.c_str()));
  double bigram = (double)(1.0 - dice(s1.c_str(), s2.c_str(), 2));
  double trigram = (double)(1.0 - dice(s1.c_str(), s2.c_str(), 3));
  double ret = trigram * 0.9 + bigram * 0.1;
  return ret;
}
